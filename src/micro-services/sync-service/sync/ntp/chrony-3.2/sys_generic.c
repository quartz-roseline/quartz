...
apply_step_offset(double offset)
{
  ...
  printf("setting CLOCK_REALTIME\n");
    ...
    #ifdef QOT_TIMELINE_SERVICE
      //#ifdef QUARTZ_V1
      #ifndef SYNC_PRIVELEGED
        qot_gl_timeline_gettime(&old_time);
        UTI_AddDoubleToTimespec(&old_time, -offset, &new_time);
        UTI_TimespecToTimeval(&new_time, &new_time_tv);
        if (qot_gl_timeline_settime(&new_time) < 0) {
          DEBUG_LOG("settimeofday() failed");
          return 0;
        }
        else {
          printf("[T%i] set clock to %lld.%9llu\n", global_timelineid, (long long)new_time.tv_sec, (unsigned long long)new_time.tv_nsec);
        }
      #else
        LCL_ReadRawTime(&old_time);
        UTI_AddDoubleToTimespec(&old_time, -offset, &new_time);
                               ...
                               lcl_SetSyncStatusDriver sys_set_sync_status)
{
  ...
  drv_set_sync_status = sys_set_sync_status;
    LOG(LOGS_INFO, "Initial frequency1 %.3f ppm", base_freq);

  base_freq = (*drv_read_freq)();  
  LOG(LOGS_INFO, "Initial frequency2 %.3f ppm", base_freq);
