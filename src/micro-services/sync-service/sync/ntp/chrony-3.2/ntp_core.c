...
/* ================================================== */
#ifdef NTP_QOT_STACK
void NCR_AdjustPoll(NCR_Instance inst, double current_accuracy, double required_accuracy)
{
  adjust_poll(inst, get_poll_adj(inst, current_accuracy, required_accuracy));
  return;
}
#endif
...
#ifdef QOT_TIMELINE_SERVICE
int clock_project_timeline_gl(struct timespec *ts, struct timespec *tml_ts)
{
  utimepoint_t utp; 

  timepoint_from_timespec(&utp.estimate, ts);

  qot_gl_timeline_loc2rem(&utp, 0);
               ...
               NTP_Local_Timestamp *rx_ts, NTP_Packet *message, int length)
{
  ...
  if (synced_packet && (!interleaved_packet || inst->valid_timestamps)) {
    ...
    if (delay < precision)
      ...
      /* QoT Stack Project received timestamp to timeline reference */
        ...
        printf("Remote Transmit    %lld.%9llu\n", remote_transmit.tv_sec, remote_transmit.tv_nsec);
        printf("Local Receive Core %lld.%9llu\n", local_receive.ts.tv_sec, local_receive.ts.tv_nsec);
        printf("Local Receive TL   %lld.%9llu\n", local_receive_tml.tv_sec, local_receive_tml.tv_nsec);
        printf("[T%i]: fractional delay          = %f\n", global_timelineid, delay);
        printf("[T%i]: fractional seconds offset = %f\n", global_timelineid, offset);
      #else
        // #ifdef QUARTZ_V1
        #ifndef SYNC_PRIVELEGED // write the sync parameters to the shared-memory region
          if(clock_project_timeline_gl(&local_receive.ts, &local_receive_tml)){
            printf("[T%i]: timeline projection failed", global_timelineid);
            return QOT_RETURN_TYPE_ERR;
          }
          ...
          printf("[T%i]: fractional delay          = %f\n", global_timelineid, delay);
          printf("[T%i]: fractional seconds offset = %f\n", global_timelineid, offset);
        #else // discipline CLOCK_REALTIME
          /* Calculate offset.  Following the NTP definition, this is negative
           if we are fast of the remote source. */
          offset = UTI_DiffTimespecsToDouble(&remote_average, &local_average);
          printf("[T%i]: fractional delay          = %f\n", global_timelineid, delay);
  ...
  if (valid_packet) {
    ...
    if (good_packet) {
      ...
      #ifdef NTP_QOT_STACK
      // Modify the poll adj to track the QoT estimates -> initial implementation
