...
/* Added for the QoT Stack */
#ifdef NTP_QOT_STACK
int SRC_GetBestSource(char* source_ip, int *stratum, int buf_size)
{
    char* invalid = "invalid";
    if (selected_source_index == INVALID_SOURCE)
    {
      strncpy(source_ip, invalid, sizeof(invalid));
      return -1;
    }
    else
    {
      char *ip = source_to_string(sources[selected_source_index]);
      strcpy(source_ip, ip);
      *stratum = sources[selected_source_index]->sel_info.stratum;
    }
    return 0;
}

int SRC_GetBestSourceIPAddr(IPAddr *ip_addr)
{
    char* invalid = "invalid";
    if (selected_source_index == INVALID_SOURCE)
    {
    ...
    {
      *ip_addr = *(sources[selected_source_index]->ip_addr);
    }
    return 0;
}
#endif
