...
/* Added for the QoT Stack */
#ifdef NTP_QOT_STACK
int NSR_AdjustPoll(IPAddr *ip_addr, double current_accuracy, double required_accuracy)
{
  int slot, found = 0;
  SourceRecord *record;
  uint32_t hash;
  ...
  for (i = 0; i < size / 2; i++) {
    ...
    if (!UTI_CompareIPs(&record->remote_addr->ip_addr, ip_addr, NULL)) 
    {
      found = 2;
      break;
    }
  ...
  { 
    record = get_record(slot);
    /* Adjust the Polling */
    NCR_AdjustPoll(record->data, current_accuracy, required_accuracy);
    return 0;
  }
  return -1;
}
