				   ...
				   Integer64 correction2)
{
	...
	double dmax, dmin;
	//qot_bounds_t bounds;// Commented out by Anon
	int fd;
	...
	// Added by Anon -> Calculate ingress timeline timestamp (shifting from feed-forward to feedback)
	...
	// Commented out by Anon (Old code written by Fatima)
	...
	case SERVO_LOCKED:
		...
		*/
		...
		// bounds.u_drift = (s32) -(freq_stats.mean - (0.5*freq_stats.stddev)); // Commented out by Anon
		// bounds.l_drift = (s32) -(freq_stats.mean + (0.5*freq_stats.stddev)); // Commented out by Anon
		...
		// bounds.u_nsec = -(offset_stats.rms - offset_stats.stddev); // Commented out by Anon
		// bounds.l_nsec = -(offset_stats.rms + offset_stats.stddev); // Commented out by Anon
		...
		// fd = CLOCKID_TO_FD(s->tml_clkid); // Commented out by Anon

		// if(ioctl(fd, TIMELINE_SET_SYNC_UNCERTAINTY, &bounds)){ // Commented out by Anon
		// 	pr_warning("Setting sync uncertainty failed for timeline "); // Commented out by Anon
		// } // Commented out by Anon
		/* SET UNCERTAINTY END */
