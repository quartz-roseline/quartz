...
/* New header for the Quartz Timeline Service */
#include "../local_timeline.h"

#ifdef PTP_QUARTZ
/* Added PTP Timeline clock ops */
...
struct clock {
	...
	#else
	tl_translation_t* tml_clk_params;	/* Timeline clock parameters */ 
	#endif
};
...
#ifdef PTP_QUARTZ
// Flag indicating if the clock is the master for this timeline
int timeline_master_flag[MAX_TIMELINES];
#endif

struct clock *clock_create(enum clock_type type, struct config *config,
			   const char *phc_device, int timelineid, int *timelinesfd, void* clk_params)
{
	enum timestamp_type timestamping =
	...
	#ifdef PTP_QUARTZ
	// Set the timeline node is master flag to 0
	timeline_master_flag[c->timelineid] = 0;
	#endif
	...
	/* Added for the QoT Stack */
	#ifdef PTP_QUARTZ
	/* Register the identity of the PHC clock with the timeline clock adjust subsystem */
	qot_set_phc(c->clkid);
	#endif
	c->utc_offset_set = 0;
	c->leap_set = 0;
	...
	#ifdef PTP_QUARTZ
	c->tml_clk_params = (tl_translation_t*) clk_params;
	#else
...
#ifdef PTP_QUARTZ
/* Quartz function to project core time to timeline time */
int clock_project_timeline(tl_translation_t* clk_params, tmv_t ts, struct timespec *tml_ts)
{
	utimepoint_t utp; 

	TP_FROM_nSEC(utp.estimate, ts);

	qot_loc2rem(&utp, 0, clk_params);
	tml_ts->tv_sec = utp.estimate.sec;
    tml_ts->tv_nsec= utp.estimate.asec / NS_PER_SEC;
	return 0;
}

/* Quartz function to set the timeline frequency */
void clock_timeline_set_freq(tl_translation_t* clk_params, double freq)
{
	struct timex tx;
	...
	if (qot_timeline_adjtime(&tx, clk_params) < 0)
		pr_err("failed to adjust the clock: %m");
}

/* Quartz function to step the timeline clock */
void clock_timeline_step(tl_translation_t* clk_params, int64_t step)
{
	struct timex tx;
	int sign = 1;
	if (step < 0) {
...
enum servo_state clock_synchronize(struct clock *c, tmv_t ingress, tmv_t origin)
{
	...
	#ifdef PTP_QUARTZ
	if(clock_project_timeline(c->tml_clk_params, ingress, &ingress_tml)){
	...
	case SERVO_UNLOCKED:
		break;
	case SERVO_JUMP:
		/* Changes for QoT Stack */
		#ifdef PTP_QUARTZ
		clock_timeline_set_freq(c->tml_clk_params, -adj);
		clock_timeline_step(c->tml_clk_params, -tmv_to_nanoseconds(c->master_offset));
		#else
...
static void handle_state_decision_event(struct clock *c)
{
	...
	LIST_FOREACH(piter, &c->ports, list) {
		...
		case PS_GRAND_MASTER:
			#ifdef PTP_QUARTZ
			timeline_master_flag[c->timelineid] = 1;
			#endif
